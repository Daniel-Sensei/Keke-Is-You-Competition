#!/usr/bin/env python3
"""
Direct comparison with original agent to understand regressions
"""
import sys
import os
sys.path.append(os.path.dirname(os.path.abspath(__file__)))

# Backup the current improved agent
import shutil
shutil.copy('agents/astar_chat_AGENT.py', 'agents/astar_chat_AGENT_improved.py')

# Test both versions
from baba import parse_map, GameState, make_level
import json

def test_level_1_specifically():
    """Focus specifically on level 1 to understand the regression"""
    print("ANALYZING LEVEL 1 REGRESSION")
    print("=" * 40)
    ...
    print(f"Level 1 ASCII:")
    print(level1['ascii'])
    print(f"Expected solution: {level1['solution']} ({len(level1['solution'])} moves)")
    ...
    # Analyze the heuristic values step by step
    print(f"\nHeuristic analysis:")
    ...
    print(f"\nTesting with different iteration limits:")
    for iterations in [50, 100, 200, 300, 500]:
        solution = agent.search(initial_state.copy(), iterations)
        if solution:
            print(f"  {iterations} iterations: ✓ Found solution with {len(solution)} moves")
            break
        else:
            print(f"  {iterations} iterations: ✗ No solution")
    
    # Test the expected solution manually
    print(f"\nVerifying expected solution works:")
    from baba import advance_game_state, check_win, Direction
    test_state = initial_state.copy()
    
    for i, char in enumerate(level1['solution']):
        direction = Direction(char.lower())
        if direction != Direction.Undefined:
            test_state = advance_game_state(direction, test_state)
            print(f"  After move {i+1} ({char}): Rules = {test_state.rules[:3]}...")
            if check_win(test_state):
                print(f"  ★ WIN achieved after {i+1} moves!")
                break

def create_minimal_working_agent():
    """Create a minimal agent that definitely works for level 1"""
    print(f"\n" + "="*40)
    print("CREATING MINIMAL WORKING AGENT")
    
    minimal_agent_code = '''# agents/minimal_astar_AGENT.py
...
'''
    ...
    with open('agents/minimal_astar_AGENT.py', 'w') as f:
        f.write(minimal_agent_code)
    
    print("Created minimal_astar_AGENT.py")
    ...
    minimal_agent = MinimalASTARAgent()
    solution = minimal_agent.search(initial_state, 300)
    
    if solution:
        print(f"✓ Minimal agent solved level 1 with {len(solution)} moves")
        return True
    else:
        print(f"✗ Even minimal agent failed level 1")
        return False

if __name__ == "__main__":
    test_level_1_specifically()
    create_minimal_working_agent()
