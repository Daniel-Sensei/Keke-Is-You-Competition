...
class FinalASTARAgent(BaseAgent):
    ...
    def _heuristic_distance(self, state: GameState) -> float:
        ...
        # Calcolo distanza più sofisticato per livelli lunghi
        distances = []
        ...
        # Adaptive distance weighting based on search depth
        if self.search_depth > 15:  # Deep search, be more optimistic
            return min_dist * 0.8
        else:
            return min_dist
    ...
    def _heuristic_rules(self, state: GameState) -> float:
        ...
        if not any("win" in r for r in rules):
            # Try multiple possible win rule formations
            possible_win_objects = ["flag", "baba", "rock", "keke"]
            win_costs = []
            for obj in possible_win_objects:
                if self._get_word_objects_by_base_name(state, obj):
                    win_costs.append(self._estimate_rule_formation_cost(state, obj, "is", "win"))
    ...
    def _calculate_heuristic(self, state: GameState) -> float:
        ...
        # Adaptive weights based on game progress and search depth
        if num_rules < 2:
            w_rules = 1.5
            w_dist = 0.7
        elif not state.winnables:
            ...
            # In late game, adjust based on search depth
            if self.search_depth > 20:
                w_rules = 0.6
                w_dist = 1.4  # More aggressive distance seeking
            else:
                w_rules = 0.8
                w_dist = 1.2
        ...
        # Check for terminal losing states
    ...
    def search(self, initial_state: GameState, iterations: int = 400) -> List[Direction]:
        """Enhanced A* search with iterative deepening for difficult levels"""
        
        # First attempt with standard search
        result = self._search_standard(initial_state, iterations // 2)
        if result:
            return result
            
        # If failed, try with iterative deepening
        return self._search_iterative_deepening(initial_state, iterations // 2)

    def _search_standard(self, initial_state: GameState, iterations: int) -> List[Direction]:
        """Standard A* search"""
        self.search_depth = 0
        ...
        while open_set and iterations > 0:
            ...
            if len(path) >= 35:  # Max path length
                continue
    ...
    def _search_iterative_deepening(self, initial_state: GameState, iterations: int) -> List[Direction]:
        """Iterative deepening search for complex levels"""
        ...
        for max_depth in max_depths:
            ...
            result = self._search_depth_limited(initial_state, max_depth, iterations // len(max_depths))
            if result:
                return result
                
        return []

    def _search_depth_limited(self, initial_state: GameState, max_depth: int, iterations: int) -> List[Direction]:
